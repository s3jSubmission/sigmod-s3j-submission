        ...
        if item.key.decode() == "running_job_id":
            ...
            if job_uuid != stopped_job:
                if stats_key == "final-comps-per-machine":
                    ...
                    print("Received item: ", item)
                    if item.value.decode() == "done":
                        print(item.value.decode() == "done")
...
# TODO:
#   - Fix MR-Partition: Improve the optimal estimation or terminate earlier/assign arbitrarily the extra big groups. - DONE
#   - Fix IMLP: penalization and its indices. - DONE
