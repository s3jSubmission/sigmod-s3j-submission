...
if __name__ == "__main__":
    ...
    join_jobid_request = requests.get(f'http://coordinator:5000/get_join_jobid')
    job_id = join_jobid_request.text
    ...
    r = requests.get(f'http://flink-rest:8081/jobs/{job_id}/plan')
    ...
    while True:
        ...
        for subtask_id in range(parallelism):
            b = requests.get(f'http://flink-rest:8081/jobs/{job_id}/vertices/{vertix_id}/subtasks/{subtask_id}/metrics'
                             f'?get={metrics}')
        ...
        for subtask_id in range(parallelism):
            for idx in range(len(metrics_returned[subtask_id])):
                if float(metrics_returned[subtask_id][idx]["value"]) != 0.0:
                    if not started:
                        started = True
        ...
        if started:
            ...
            for subtask_id in range(parallelism):
                for metric in metrics_dict[subtask_id].keys():
                    if metrics_dict[subtask_id][metric][-1] != 0.0:
                        all_zero = False
                        break
